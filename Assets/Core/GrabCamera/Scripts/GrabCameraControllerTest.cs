...
{
    public class GrabCameraControllerTest : MonoBehaviour
    {
        #region Statements
        ...
        private void MobilePan(Vector2 touchDelta)
        {
            if (!touchDelta.y.Equals(0) || !touchDelta.x.Equals(0))
            {
                var mouseRay = _camera.ScreenPointToRay(Input.mousePosition);
                var planeY = new Plane(Vector3.up, Vector3.zero);
        ...
        private void Update()
        {
            if (SystemInfo.deviceType == DeviceType.Handheld)
            {
                if (Input.touchCount > 0)
                {
                    ...
                    {
                        var touchDelta = touch.position - touchStart;

                        // Si le déplacement est suffisamment important pour être considéré comme un swipe
